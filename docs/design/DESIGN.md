...
- [x] Mobile — bottom tab bar with capture FAB, 72px document cards, filter
      chips over the system picker
...
- [x] Documents list — one sortable table card, chip filter bar, server-side
      pagination with page size and totals, multi-select with the dark action

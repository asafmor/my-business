...
describe("currentReportingMonth", () => {
  it("formats the given date as YYYY-MM in UTC", () => {
    expect(currentReportingMonth(new Date("2026-09-14T12:00:00Z"))).toBe("2026-09");
  });

  it("pads single-digit months", () => {
    expect(currentReportingMonth(new Date("2026-01-05T00:00:00Z"))).toBe("2026-01");
  });

  it("rolls over at the year boundary using UTC, not local time", () => {
    expect(currentReportingMonth(new Date("2025-12-31T23:30:00Z"))).toBe("2025-12");
    expect(currentReportingMonth(new Date("2026-01-01T00:30:00Z"))).toBe("2026-01");
  });
});
...
const { DrizzleDashboardRepository } = await import("../src/server/documents/dashboard-repository");
...
describe("DrizzleDashboardRepository.summary", () => {
  it("returns the current-month aggregate row", async () => {
    const selectChain = chain([
      { documentCount: 3, needsReviewCount: 1, totalExpenses: "150.00", vatTotal: "25.00" },
    ]);
